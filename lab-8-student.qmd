...
library(tidyverse)
library(dplyr)
data = read.csv("Data/BlackfootFish.csv")
data %>%
...
library(ggplot2)
dataNA <- filter(data, is.na(weight))
dataNA$Section = dataNA$section
ggplot(data = dataNA, aes(x = year, y = trip, color = Section)) +
  geom_point() +
  geom_jitter() +
  scale_y_discrete(limits = c("1", "2")) +
  labs(x = "Year", y = "Section", title = "Missing values by year, section, and trip", subtitle = "From 1989-2007, between Scotty Brown and Johnsrud in 2 sections")
```
...
> The second normalization uses length as the minimum in the denominator, and not weight
...
rescale_01 <- function(vector) {
  if (!is.numeric(vector) | (length(vector) < 2)) {
    stop("Not numeric or length of 1")
  }
  scaledVector <- (vector - min(vector, na.rm = TRUE)) / (max(vector, na.rm = TRUE) - min(vector, na.rm = TRUE))
  return(scaledVector)
}
```
