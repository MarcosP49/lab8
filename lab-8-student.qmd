...
```{r}
#| label: load-data
data_fish <- read_csv("Data/BlackfootFish.csv", show_col_types = FALSE)
```
...
dataNA <- filter(data_fish, is.na(weight)) %>%
  mutate(Section = section)

summary_data <- data_fish %>%
  filter(is.na(weight)) %>%
  group_by(year, section) %>%
  summarise(n_missing = n(), .groups = "drop")

ggplot(summary_data, aes(x = year, y = n_missing, fill = section)) +
  geom_col(position = "dodge") +
  labs(
    x = "Year",
    y = "Number of Missing Weight Values",
    title = "Missing Weights by Year and Section"
  ) +
  theme_minimal()
```
